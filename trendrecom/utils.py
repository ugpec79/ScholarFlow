...
def generate_trending_papers():
    ...
    papers['publish_date'] = pd.to_datetime(papers['year'].astype(str) + "-01-01", errors='coerce')
    papers.rename(columns={'_id': 'paper_id', 'n_citation': 'citation_count'}, inplace=True)
    ...
    if has_user_data:
        ...
        if not required_cols.issubset(actions.columns):
            print("⚠️ user_actions.csv is missing required columns. Skipping interaction analysis.")
            has_user_data = False
    else:
        print("ℹ️ No user_actions.csv found or file is empty. Proceeding without user interactions.")
    ...
    if has_user_data:
        ...
        comments['comment'] = comments['comment'].apply(lambda x: x if isinstance(x, str) else "")
        ...
        # Aggregation
        interaction_counts = actions.groupby(['paper_id', 'action']).size().unstack(fill_value=0)
        sentiment_summary = comments.groupby('paper_id')['sentiment_score'].mean().reset_index().rename(columns={"sentiment_score": "avg_sentiment"})
        comment_counts = comments.groupby('paper_id').size().reset_index(name='comment_count')
    else:
    ...
    paper_data[['LIKE', 'DISLIKE', 'CLICKED', 'comment_count', 'avg_sentiment']] = \
        paper_data[['LIKE', 'DISLIKE', 'CLICKED', 'comment_count', 'avg_sentiment']].fillna(0)

    # --- Step 5: Compute Raw Feedback & Citation Scores ---
    paper_data['raw_citation_score'] = paper_data['citation_count'].apply(lambda x: log(x + 1))
    ...
    paper_data['citation_score'] = (paper_data['raw_citation_score'] - citation_min) / (citation_max - citation_min + 1e-9)

    feedback_min = paper_data['raw_feedback_score'].min()
    feedback_max = paper_data['raw_feedback_score'].max()
    paper_data['feedback_score'] = (paper_data['raw_feedback_score'] - feedback_min) / (feedback_max - feedback_min + 1e-9)
    ...
    paper_data['trending_score'] = (
        (paper_data['feedback_score'] * feedback_weight + paper_data['citation_score'] * citation_weight)
        * paper_data['recency_weight']
    ).round(4)

    # --- Step 7: Sort + Export ---
    top_trending = paper_data.sort_values(by='trending_score', ascending=False)
    top_trending[['paper_id', 'title', 'trending_score']].to_csv("trending_papers.csv", index=False)
